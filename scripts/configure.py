...
and this script writes your `deploy.conf` and switches the right k8s
manifests on.

Usage:
    python3 scripts/configure.py             # interactive wizard
    python3 scripts/configure.py --yes       # no prompts (deploy.conf / env vars)
...
# Everything written to deploy.conf, in this order (EXTRA_ENV, SECRETS last).
KEYS = [
...
def value_for(key: str) -> str:
    """Env var, else deploy.conf, else default. For prompts & --yes mode."""
    val = env(key)
    if val:
        return val
...
def has_value(key: str) -> bool:
    """True if the key is explicitly set (env var or config file), not a default."""
    return bool(env(key)) or bool(dotenv_value(key))


def load_dotenv_values() -> dict:
    """Parse deploy.conf (fallback: legacy .env) without letting it override
    ...
    for line in source.read_text().splitlines():
        ...
        if key and key not in values:
            ...
            val = val.strip().split(" #", 1)[0].split("\t#", 1)[0].strip()
...
def secret_value(name: str) -> str:
    """A secret's value: real env var, else config file."""
...
def write_env(settings: dict):
    ...
    lines = [
        ...
        "# Re-run 'make configure' to change these. Commit this file — the",
        "# pipeline reads it. Sensitive values stay in CI/CD variables.",
    ...
    print(f"\n==> Saved your configuration to {CONFIG_FILE.name}")
...
def run_wizard() -> dict:
    ...
    print("  (For local testing, put them in deploy.conf like DB_PASSWORD=mypass)")
...
def main() -> int:
    ...
    parser.add_argument("--yes", action="store_true",
                        help="no prompts — use deploy.conf / environment values")
    ...
    if args.yes:
        ...
        if errors:
            ...
            for e in errors:
                print(f"  - {e}")
            print("Fix the values above (set them in deploy.conf or as "
